...
- **Secret-handling standardization — goal stated 2026-08-22, not yet a Constitution commitment.** Paulo wants to move past the v2.3.0 open question (Pillar 1) toward an actual standardized procedure across his own work, not leave it flagged indefinitely. The concrete target: a repeatable flow where Paulo pastes a new credential into a staging location only he touches, then simply asks the AI to carry out the rotation/set from there — the AI does the mechanical work, but the value itself never has to be typed into the conversation to get that convenience. Refined understanding from the conversation that surfaced it, kept here rather than in `CONSTITUTION.md` since it's reasoning trail, not yet a settled principle: cleanup after use (deleting a staged file, unsetting an env var) only closes the disk-residency window — it doesn't undo exposure that already happened if the secret ever became text an AI processed. A file-staged value is only actually safe from the AI channel when all of: the human stages the file/env var himself, never dictating the value in a chat message; the command referencing it uses a static path (`--data-file=path`), never an inline-expanded variable (`--data=$VAR`), since shell tracing/echo could surface the expanded value in what comes back; nothing in the command chain — success or error path — prints or dumps the file's contents; and the AI is never asked to verify the file by reading it. Separately, real exposure channels apply regardless of AI involvement: shell history, terminal scrollback, `ps aux` during execution, and `rm` not being a secure erase. Open: whether/how this graduates into an adopted Pillar 1 practice.
